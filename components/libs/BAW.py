...
def _standardNormalPDF(x):
    val = (1 / (2 * _cm.pi) ** 0.5) * _m.exp(-1 * (x ** 2) / 2)
    return val
...
def _priceEuropeanOption(option_type_flag, S, X, T, r, b, v):
    ...
    if option_type_flag == CALL:
        bsp = S * _m.exp((b - r) * T) * _standardNormalCDF(d1) - X * _m.exp(
            -r * T
        ) * _standardNormalCDF(d2)
    else:
        bsp = X * _m.exp(-r * T) * _standardNormalCDF(-d2) - S * _m.exp(
            (b - r) * T
        ) * _standardNormalCDF(-d1)
...
def _approximateAmericanCall(S, X, T, r, b, v):
    ...
    if b >= r:
        ...
        Sk = _Kc(X, T, r, b, v)
        N = 2 * b / v ** 2
        k = 2 * r / (v ** 2 * (1 - _m.exp(-1 * r * T)))
        d1 = (_m.log(Sk / X) + (b + (v ** 2) / 2) * T) / (v * (T ** 0.5))
...
def _Kc(X, T, r, b, v):
    ...
    LHS = Si - X
    RHS = (
        _priceEuropeanOption(CALL, Si, X, T, r, b, v)
        + (1 - _m.exp((b - r) * T) * _standardNormalCDF(d1)) * Si / Q2
    )
    bi = (
        _m.exp((b - r) * T) * _standardNormalCDF(d1) * (1 - 1 / Q2)
        + (1 - _m.exp((b - r) * T) * _standardNormalPDF(d1) / (v * (T) ** 0.5)) / Q2
    )
    ...
    while abs(LHS - RHS) / X > E:
        ...
        bi = (
            _m.exp((b - r) * T) * _standardNormalCDF(d1) * (1 - 1 / Q2)
            + (1 - _m.exp((b - r) * T) * _standardNormalCDF(d1) / (v * (T) ** 0.5)) / Q2
        )
...
def _Kp(X, T, r, b, v):
    ...
    LHS = X - Si
    RHS = (
        _priceEuropeanOption(PUT, Si, X, T, r, b, v)
        - (1 - _m.exp((b - r) * T) * _standardNormalCDF(-1 * d1)) * Si / Q1
    )
    bi = (
        -1 * _m.exp((b - r) * T) * _standardNormalCDF(-1 * d1) * (1 - 1 / Q1)
        - (1 + _m.exp((b - r) * T) * _standardNormalPDF(-d1) / (v * (T) ** 0.5)) / Q1
    )
    ...
    while abs(LHS - RHS) / X > E:
        ...
        bi = (
            -_m.exp((b - r) * T) * _standardNormalCDF(-1 * d1) * (1 - 1 / Q1)
            - (1 + _m.exp((b - r) * T) * _standardNormalCDF(-1 * d1) / (v * (T) ** 0.5))
            / Q1
        )
...
def _checkBadNumericInput(
    spot_price,
    strike_price,
    expiration_time_in_years,
    interest_rate_dec_pa,
    carry_rate_dec_pa,
    volatility_dec_pa,
):
    ...
    if interest_rate_dec_pa < 0 or interest_rate_dec_pa >= 1.00:
        raise ValueError(
            "Interest rate in annualized decimal format must be >= 0 and < 1.00"
        )
    if carry_rate_dec_pa < 0 or carry_rate_dec_pa >= 1.00:
        raise ValueError(
            "Carry Rate in annualized decimal format must be >= 0 and < 1.00"
        )
    if volatility_dec_pa < 0 or volatility_dec_pa >= 10.00:
        raise ValueError(
            "Volatility in annualized decimal format must be > 0 and < 10.00 "
        )
...
def getValue(
    ...
    if output_flag == PRICE:
        ...
        return (
            priceOption(option_type_flag, S + _dS, X, T, r, b, v)
            - priceOption(option_type_flag, S - _dS, X, T, r, b, v)
        ) / (2 * _dS)
    elif output_flag == GAMMA:
        return (
            priceOption(option_type_flag, S + _dS, X, T, r, b, v)
            - 2 * priceOption(option_type_flag, S, X, T, r, b, v)
            + priceOption(option_type_flag, S - _dS, X, T, r, b, v)
        ) / _dS ** 2
    elif output_flag == VEGA:
        return (
            priceOption(option_type_flag, S + _dS, X, T, r, b, v + _dV)
            - priceOption(option_type_flag, S + _dS, X, T, r, b, v - _dV)
        ) / 2
    elif output_flag == THETA:
        return priceOption(
            option_type_flag, S + _dS, X, T - _dT, r, b, v
        ) - priceOption(option_type_flag, S + _dS, X, T, r, b, v)
